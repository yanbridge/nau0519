...
##上证指数：^SSEC,000001,ss 深证成指：399001.sz,沪深300指数代码：000300.ss
...
setSymbolLookup(深证成指=list(name="399001.sz",src="yahoo",from = "2012-01-01",
            to = "2012-08-31"))  
getSymbols("深证成指")
chartSeries(深证成指,theme="white")
...
万科[万科==0]<-NA
万科<-na.omit(万科)
chartSeries(万科,theme="white")
dev.new()  #添加一副新图利于比较
chartSeries(万科,up.col='red',dn.col='green',theme="white") #中国式K线图
...
#日本股市数据，install.packages("RFinanceYJ")
...
##从OANDA网站获取外汇数据，get the data of exchange rate from OANDA:  http://www.oanda.com/
library(quantmod)
getSymbols("USD/RUB",src="oanda",from = "2012-04-01",to="2013-03-31")  

##将下载的数据写入Excel
write.csv(600031.ss.f,file="data.csv")  #x为保存在R里面的数据，data.csv为Excel的数据